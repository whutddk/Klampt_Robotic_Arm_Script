	...
	for r in range(0,16):
		for fi in range (0,32):
			for thr in range (0,32):
				...
				grid.loadFile("terrains/trapezoid/trapezoid" + str(r) + "_"+ str(fi)+ "_" + str(thr) +".off")
				#grid.loadFile("terrains/trapezoid/cube.off")

				grid.transform([1,0,0,0,1,0,0,0,1],[0,0,0])			

				Mesh = world.makeTerrain("Grid," + "%3d"%r + "," + "%3d"%fi + "," + "%3d"%thr)

				Mesh.geometry().set(grid)
				#if ( (z+a+r)%3 == 0 ):
				Mesh.appearance().setColor(0.5,0.1,0.1,0.6)
				#elif ( (z+a+r)%3 == 1 ):
...
def create_Edge(Index):
	...
	for k in range (0,101):
		...
		for p,q in collisionTest.robotTerrainCollisions(0):
			...
			thr = int(result[13:16])
			oneEdge[1024*r+32*fi+thr] = 1
			cnt = cnt + 1;
