	...
	for rad in range(0,32):
		for fi in range (0,32):
			with open('./trapezoid'+ str(x)+ '_'+ str(rad) +"_" +str(fi) +'.off','w') as modelFile:
				radius = 0.1421 + 0.023*x
				theta = -1.57 + 0.098*rad 
				# fieta = -1.1442 + 0.075*fi
				fieta = -1.57 + 0.098*fi

				X1_1 = radius * math.cos(theta) * math.cos(fieta)-0.325
				Y1_1 = radius * math.sin(theta) * math.cos(fieta)
				Z1_1 = radius * math.sin(fieta)

				X1_2 = radius * math.cos(theta) * math.cos(fieta+0.098)-0.325
				Y1_2 = radius * math.sin(theta) * math.cos(fieta+0.098)
				Z1_2 = radius * math.sin((fieta+0.098) ) 



				X2_1 = (radius + 0.023) * math.cos(theta) * math.cos(fieta)-0.325
				Y2_1 = (radius + 0.023) * math.sin(theta) * math.cos(fieta)
				Z2_1 = (radius + 0.023) * math.sin(fieta)

				X2_2 = (radius + 0.023) * math.cos(theta) * math.cos(fieta + 0.098)-0.325
				Y2_2 = (radius + 0.023) * math.sin(theta) * math.cos(fieta + 0.098)
				Z2_2 = (radius + 0.023) * math.sin(fieta+0.098)



				X3_1 = radius * math.cos(theta + 0.098) * math.cos(fieta)-0.325
				Y3_1 = radius * math.sin(theta + 0.098) * math.cos(fieta)
				Z3_1 = radius * math.sin(fieta)

				X3_2 = radius * math.cos(theta+0.098) * math.cos( fieta + 0.098 )-0.325
				Y3_2 = radius * math.sin(theta+0.098) * math.cos( fieta + 0.098 )
				Z3_2 = radius * math.sin(fieta+0.098)



				X4_1 = (radius + 0.023) * math.cos(theta+0.098) * math.cos(fieta )-0.325
				Y4_1 = (radius + 0.023) * math.sin(theta+0.098) * math.cos(fieta )
				Z4_1 = (radius + 0.023) * math.sin(fieta)

				X4_2 = (radius + 0.023) * math.cos(theta+0.098) * math.cos(fieta + 0.098)-0.325
				Y4_2 = (radius + 0.023) * math.sin(theta+0.098) * math.cos(fieta + 0.098)
				Z4_2 = (radius + 0.023) * math.sin(fieta+0.098)
